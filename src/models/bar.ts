...
export class Bar {
	...
	getStaveNotes(scale: number, duration: string, key: string, color: string = 'black'): Array<Vex.Flow.StaveNote> {
		const makeRest = () => {
		...
		for (let index: number = 0; index < this.length * scale; index++) {
			notes.push(makeRest());
		}
		...
		this.beats.forEach((beat: Beat, index: number) => {
			...
			if (beat.note === Note.Accent) {
				...
				let marcato = new Vex.Flow.Articulation('a^').setPosition(Vex.Flow.Modifier.Position.ABOVE);
